...
export default function ViewerBracketMatchCard({ match, teams, userPick, onPick, getTeamById }: ViewerBracketMatchCardProps) {
  ...
  const cardClasses = `w-[7rem] h-[60px] flex flex-col justify-between relative p-0.5`; 

  return (
    <div className={`${cardClasses}`}>
      <div className="space-y-0.5"> {/* Further reduced space-y */}
        <div
          className={`flex items-center justify-between py-0 px-0.5 rounded-md transition-colors duration-150 ${ // Further adjusted padding
        ...
        >
          <div className="flex items-center gap-0.5"> {/* Further reduced gap */}
            ...
            <h3 className="text-[0.65rem] font-semibold text-white">{team1?.name || 'TBD'}</h3> {/* Reduced text size */}
          </div>
          {isTeam1Picked && <CheckCircle className="w-2.5 h-2.5 text-blue-400" />} {/* Reduced icon size */}
        </div>

        <div className="flex justify-center py-0"> {/* Further adjusted padding */}
          <span className="text-[0.6rem] font-semibold text-slate-400">VS</span> {/* Reduced text size */}
        </div>
        ...
        >
          <div className="flex items-center gap-0.5"> {/* Further reduced gap */}
            ...
            <h3 className="text-[0.65rem] font-semibold text-white">{team2?.name || 'TBD'}</h3> {/* Reduced text size */}
          </div>
          {isTeam2Picked && <CheckCircle className="w-2.5 h-2.5 text-blue-400" />} {/* Reduced icon size */}
        </div>
      </div>

      {userPick && (
        <div className="mt-0.5 pt-0.5 border-t border-slate-600"> {/* Adjusted margin and padding */}
          <div className="flex items-center justify-center gap-0.5 text-[0.6rem] font-semibold text-blue-400"> {/* Reduced gap and text size */}
            <Trophy className="w-2 h-2" /> {/* Reduced icon size */}
